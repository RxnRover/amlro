...
from amlro.ml_models import ml_models
...
class optimizer:
    ...
    def model_training(
        x_train: pd.DataFrame, y_train: pd.DataFrame, model="gb"
    ):
        ...
        regr, param_grid = ml_models.get_regressor_model(model)
    ...
    def predict_next_parameters(
        ...
        if len(config["directions"]) == 1:
            ...
            best_parameters = best_parameters.drop(
                config["objectives"][0], axis=1
            )
            ...
            best_parameters = pd.DataFrame(
                best_parameters, columns=data.columns
            )
    ...
    def get_optimized_parameters(
        ...
        parameters_encoded, parameters_decoded = (
            optimizer.stringify_parameters_objectives(
        ...
        if len(parameters_list) != 0:
            optimizer.write_data_to_training(
                reaction_data_path, parameters_encoded
            )
            optimizer.write_data_to_training(
                reaction_data_decoded_path, parameters_decoded
            )
        ...
        if len(config["objectives"]) == 1:
            regr = optimizer.model_training(x_train, y_train, model)
        else:
            regr = optimizer.mo_model_training(x_train, y_train, model)

        # Predict the next best reaction conditions.
        best_combo = optimizer.predict_next_parameters(
            regr, data, config, batch_size
        )
        ...
        for _, conditions in best_combo.iterrows():
            ...
            next_best_conditions.append(
                optimizer.categorical_feature_decoding(config, conditions_list)
            )
    ...
    def stringify_parameters_objectives(
        ...
        for parameters, objectives in zip(parameters_list, objectives_list):
            encoded = optimizer.categorical_feature_encoding(config, parameters)
